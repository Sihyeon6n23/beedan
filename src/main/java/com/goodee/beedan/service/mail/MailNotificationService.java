...
public class MailNotificationService {
    ...
    public void sendNotification(MailRequest request) {
        ...
        String subject = switch (request) {
            ...
            case BizValidationRejectMailRequest p -> "[BEEDAN] 사업자 인증 반려 안내";
        };

        String content = switch (request) {
            case PasswordResetMailRequest p -> buildPasswordResetHtml(p);
            case BizValidationRejectMailRequest p -> buildBizValidationRejectHtml();
        };
    ...
    private String buildBizValidationRejectHtml() {
        return """
    <div style="font-family: sans-serif; line-height: 1.6;">
        <h1 style="color: #333;">사업자 인증 반려 안내</h1>
        <p>회원님, 안녕하세요.</p>
        <p>첨부된 파일과 입력해주신 사업자 정보가 일치하지 않아 반려처리 되었습니다.</p>
        <p>사업자 인증이 필요하실 경우 첨부파일을 해당 이메일로 다시 보내주시기 바랍니다.</p>
    </div>
    """;
    }
}
